...
| `tracks[].title` | optional plain text, shown as written. The order of `tracks` does not decide play order: the engine draws each pick at random from the tracks not yet played (`0x6bcdfc` on 2.31, confirmed in game on Body Heat and PHONKWAVE with no reordering) |
